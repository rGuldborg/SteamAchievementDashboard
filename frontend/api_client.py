...
def fetch_player_achievements(steam_id: str, app_id: int) -> dict | None:
    try:
        resp = requests.get(
...
def fetch_llm_estimate(steam_id: str, app_id: int) -> dict | None:
    try:
        resp = requests.get(
...
def fetch_marks(steam_id: str, app_id: int) -> list[dict]:
    try:
